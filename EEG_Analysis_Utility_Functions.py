...
def plotSpikeDetectionResults(data, times, spikes, channels, channel_names, start_time=None, end_time=None):
    """
    ...
    for i, channel_num in enumerate(channels):
        for j, (method_name, method_spikes) in enumerate(spikes[channel_num].items()):
            ...
            if start_time is not None and end_time is not None:
                ...
                adjusted_spike_indices = method_spikes[spike_mask] - np.where(time_mask)[0][0]

                ax.plot(filtered_times, filtered_data, label='Data')
                ax.scatter(filtered_spike_times, filtered_data[adjusted_spike_indices], color='red', label='Spikes')
            else:
                ax.plot(times, data[channel_num], label='Data')
                ax.scatter(times[method_spikes], data[channel_num][method_spikes], color='red', label='Spikes')

            ax.set_title(f'{channel_names[channel_num]} - Channel {channel_num} - Spike Detection ({method_name})')
...
def plotChannelSegments(data, times, channel_num, segments, channel_name=None, start_time=None, end_time=None):
    """
    ...
    for i, (start, end) in enumerate(segments):
        ...
        if segment_start < segment_end:
            plt.plot(times[segment_start:segment_end], data[channel_num, segment_start:segment_end], label=f'Segment {i+1}')

    title = f'Channel {channel_num} - Data with Segments' if not channel_name else f'{channel_name} - Channel {channel_num} - Data with Segments'
    plt.title(title)
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')
    plt.legend()
...
def plotAnnotationsMatplotlib(edf_file, description=None, time_buffer=10):
    ...
    matched_annotations = False  # Flag to track if any annotations matched the description
    ...
    for index, annotation in enumerate(edf_file.annotations):
        ...
        print(f'Annotation {index + 1}: Start: {annotation["onset"]}, Duration: {annotation["duration"]}, Description: {annotation["description"]}')

        plot_start_time = max(0, annotation['onset'] - time_buffer)
        plot_end_time = annotation['onset'] + annotation['duration'] + time_buffer

        edf_file.plot(start=plot_start_time, duration=plot_end_time - plot_start_time, n_channels=min(10, len(edf_file.ch_names)), title=f'Annotation {index + 1}', scalings='auto', show_options=False)
...
def plotAnnotationsPlotly(raw_data, description=None, time_buffer=10):
    ...
    for index, annotation in enumerate(raw_data.annotations):
        ...
        end_time = min(annotation['onset'] + annotation['duration'] + time_buffer, raw_data.n_times / raw_data.info['sfreq'] - 0.001)
        ...
        data_frame = pd.DataFrame(segment_data.T, columns=segment.ch_names[:min(10, len(segment.ch_names))])
        data_frame['Time'] = segment_times

        plot_title = f'Annotation {index + 1}' + (f' - {description}' if description else '')
        figure = px.line(data_frame, x='Time', y=data_frame.columns[:-1], title=plot_title)
        figure.show()
...
def calculateERSForChannel(channel_num, data, all_baselines):
    ...
    """
    return channel_num, calculateEventRelatedSynchronization(data[channel_num], all_baselines[channel_num])
...
def calculateERSPercentageForAllChannels(data, all_baselines):
    ...
    with ThreadPoolExecutor() as executor:
        results = executor.map(lambda channel_num: calculateERSForChannel(channel_num, data, all_baselines), range(data.shape[0]))
...
def groupAndCreateBipolarChannels(channels):
    ...
    for channel in channels:
        ...
        if current_group is not None and group != current_group and (group + "'") != current_group and ("'" + group) != current_group:
            ...
            for i in range(len(group_channels) - 1):
                if group_channels[i][1] + 1 == group_channels[i + 1][1]:
                    bipolar_channels.append(f'{group_channels[i][0]}-{group_channels[i + 1][0]}')
            group_channels = []
        current_group = group
...
def processEEGChunkOptimized(chunk_data, info, notch_freqs, l_freq, h_freq):
    ...
    if notch_freqs is not None:
        raw_chunk.notch_filter(notch_freqs, method='fir', fir_design='firwin', verbose=False)
    # Apply band-pass filter
    raw_chunk.filter(l_freq, h_freq, method='fir', fir_design='firwin', l_trans_bandwidth='auto', h_trans_bandwidth='auto', verbose=False)
...
def processAndStoreEEGChunk(chunk_data, info, notch_freqs, l_freq, h_freq, h5_file, start_sample):
    """
    ...
    if notch_freqs is not None:
        ...
        h5_file['notch_filtered'][:, start_sample:start_sample + notch_filtered.shape[1]] = notch_filtered

    # Apply band-pass filter and save
    bandpass_filtered = raw_chunk.copy().filter(l_freq, h_freq, method='fir', fir_design='firwin', l_trans_bandwidth='auto', h_trans_bandwidth='auto', verbose=False).get_data().astype(np.float32)
    h5_file['bandpass_filtered'][:, start_sample:start_sample + bandpass_filtered.shape[1]] = bandpass_filtered

    # Apply Hilbert transform to get the analytic signal and save
    hilbert_transformed = raw_chunk.copy().apply_hilbert(envelope=True, verbose=False).get_data().astype(np.float32)
    h5_file['hilbert'][:, start_sample:start_sample + hilbert_transformed.shape[1]] = hilbert_transformed

def processEEGDataInChunksParallel(edf_file, h5_file_path, samples_per_chunk, notch_freqs=60, l_freq=25, h_freq=250):
    """
    ...
    with h5py.File(h5_file_path, 'w') as h5_file:
        for dataset_name in ['notch_filtered', 'bandpass_filtered', 'hilbert']:
            h5_file.create_dataset(dataset_name, (edf_file.info['nchan'], len(edf_file.times)), dtype='float32')

        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(processAndStoreEEGChunk, edf_file.get_data(start=start_sample, stop=start_sample + samples_per_chunk), edf_file.info, notch_freqs, l_freq, h_freq, h5_file, start_sample) for start_sample in range(0, len(edf_file.times), samples_per_chunk)]

            for future in as_completed(futures):
                future.result()  # Wait for all futures to complete

def calculateBaselineAvoidingSpikes(data, times, annotations, spike_indices, duration, num_segments, min_segment_gap, channel_num, max_attempts):
    """
    ...
    annotation_indices = sorted([int(a * len(times) / total_duration) for a in annotations])
    baseline_segments = []
    
    attempts = 0
    start_index = 0
    while len(baseline_segments) < num_segments and start_index < len(times) - duration_indices:
        if any(start_index <= spike < start_index + duration_indices for spike in spike_indices) or any(abs(start_index - a) < min_gap_indices for a in annotation_indices):
            start_index += 1
            attempts += 1
            if attempts > max_attempts:
                print(f"Channel {channel_num}: Maximum attempts reached ({max_attempts}). Stopping search for this channel.")
                break
            continue
    ...
    baseline_segments_data = [data[channel_num][start:end] for start, end in baseline_segments]
    baseline = np.mean(baseline_segments_data)
    return baseline, baseline_segments

def calculateBaselineAndSegmentsForChannel(data, times, annotations, channel_num, spikes, duration, num_segments, min_segment_gap, max_attempts, max_retries):
    """
    ...
    while retries < max_retries:
        baseline, segments = calculateBaselineAvoidingSpikes(
            data, times, annotations, spikes_for_channel,
            duration, num_segments, min_segment_gap, 
            channel_num, max_attempts)
        ...
        duration, num_segments, min_segment_gap = duration_new, num_segments_new, min_segment_gap * 0.7
        print(f"Retrying Channel {channel_num} with adjusted parameters (attempt {retries})...")
    
    print(f"Channel {channel_num}: Maximum retries reached ({max_retries}). Setting duration to zero and returning last result.")
    return baseline, segments

def calculateBaselinesAndSegmentsForAllChannels(data, times, annotations, spikes, duration=10, num_segments=30, min_segment_gap=2, max_attempts=50000, max_retries=5):
    """
    ...
    all_baselines = {}
    all_segments = {}
    args = [(data, times, annotations, channel_num, spikes, duration, num_segments, min_segment_gap, max_attempts, max_retries) for channel_num in range(data.shape[0])]
    with ThreadPoolExecutor() as executor:
        results = executor.map(lambda x: calculateBaselineAndSegmentsForChannel(*x), args)
...
def reportChannelsWithSegmentIssues(baseline_segments, times, expected_num_segments):
    ...
    for issue_type, channels in channels_with_issues.items():
        if channels:
            issue_description = "more than the expected" if issue_type == 'extra' else "the full number of"
            print(f"\nChannels for which the function found {issue_description} segments:")
            for channel in channels:
                total_duration = sum(times[end] - times[start] for start, end in baseline_segments[channel])
                print(f"Channel {channel}: Found {len(baseline_segments[channel])} segments, expected {expected_num_segments}. Total duration of segments: {total_duration:.2f} seconds")
...
def saveEDFSegment(edf, start, end, output_path):
    ...
    channel_info = list(ThreadPoolExecutor().map(_generateChannelMetadata, range(channel_count), edf.info['ch_names'], [signal_data] * channel_count, [sampling_rate] * channel_count))
...
def extractAnnotationsToEDF(edf, annotations, description, file_prefix='buffer', buffer_secs=10, save_dir='.'):
    """
    ...
    for idx, ann in enumerate(annotations):
        ...
        if ann['description'].lower() == description.lower():
            ...
            print(f"Annotation {idx + 1}: Start: {ann['onset']}, Duration: {ann['duration']}, Description: {ann['description']}")

            # Calculate the start and end times with buffer
            start, end = max(0, ann['onset'] - buffer_secs), ann['onset'] + ann['duration'] + buffer_secs
